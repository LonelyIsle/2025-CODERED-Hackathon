...
pub async fn init_pool(pg_url: &str) -> Result<PgPool> {
    ...
    let pool = Pool::builder(mgr).max_size(8).build().context("build pool")?;
    ensure_ingested_table(&pool).await?;
    Ok(pool)
}

async fn ensure_ingested_table(pool: &PgPool) -> Result<()> {
    let client = pool.get().await.context("get conn for ensure_ingested_table")?;
